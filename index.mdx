...
<Steps>
  ...
  <Step title="Create an instance">
    ...
    The `url` is the instance's own API: the instance id is the DNS label, so `ab12cd34ef` answers at `https://ab12cd34ef.agent37.app`. That is the address you talk to next. Any other port, including the agent's dashboard, terminal, and files, is reachable at `https://ab12cd34ef-{port}.agent37.app`; see [Instance and preview URLs](/agents-api/urls).
  </Step>
  ...
  <Step title="Send it a streamed message">
    ...
    data: {"output_text":"Here is the memo...","usage":{"input_tokens":1840,"output_tokens":920,"cost_usd":0.0137},"context":{"used_tokens":22600,"window_tokens":256000}}
    ```
