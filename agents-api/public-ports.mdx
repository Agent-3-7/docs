...
- A request to a public URL [wakes a sleeping instance](/agents-api/urls#sleeping-instances-wake-on-request), which for a webhook endpoint is the feature working. The flip side: any traffic to the URL keeps an [auto-sleep](/agents-api/instances#auto-sleep) instance awake, so a scraped or polled public URL keeps the compute meter running. If that happens, delete the entry.
