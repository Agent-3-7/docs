...
<Steps>
  ...
  <Step title="Keep the site up">
    ...
    Leave `auto_sleep` off (the default) for a site that should be up around the clock. An [auto-sleep](/agents-api/instances#auto-sleep) instance does wake when the URL is visited, but its awake minutes bill at 4x the compute rate, so steady traffic on a public URL makes sleep the wrong trade.
  </Step>
</Steps>
