...
<ResponseField name="agents" type="string[]">
  The agents the template installs: `["hermes"]` on `agent37-hermes`, `["claude-code"]` on `agent37-claude-code`, `["codex"]` on `agent37-codex`, `["opencode"]` on `agent37-opencode`. Workspace templates report `[]`, because the platform does not inspect what your image installs.
</ResponseField>
