...
The `agent37-` prefix is reserved for system templates. The catalog has four entries today:
...
<ResponseField name="agents" type="string[]">
  The agents the template installs: `["hermes"]` on `agent37-hermes`, `["claude-code"]` on `agent37-claude-code`, `["codex"]` on `agent37-codex`. Workspace templates report `[]`, because the platform does not inspect what your image installs.
</ResponseField>
