...
<ParamField body="model" type="string">
  The LLM to run this turn on. Omit it to use the session's current model (the instance default on a new session). List what the instance can run with `GET /v1/models`; see [Models](/agents-api/models).
</ParamField>
...
<ResponseField name="context" type="object | null">
  The session's context window as of this turn: `{ used_tokens, window_tokens }`, the tokens occupying the model's window against the window's size. `null` when the harness didn't report a measurement.
</ResponseField>
