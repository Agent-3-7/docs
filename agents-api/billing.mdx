...
### Default and shared instances

The **default** instance type reserves the full shape around the clock, and the prices above are what it bills, flat, every month.

Most agents are idle most of the day. A **shared** instance still bursts to the whole shape whenever it works, but bills that shape's monthly average instead of the reservation, from \$1.99 per month on 2 vCPU / 4 GB and never more than the flat price. Burst capacity is best effort, so some contention is possible under sustained load. Isolation and every endpoint on this reference behave identically; only the capacity reservation changes.

Start with the default. Reach for **shared** when cost is what you are optimizing: pass `"type": "shared"` on [create](/agents-api/instances#create-an-instance). Shared is enabled per workspace; until yours is, create returns `403 shared_unavailable` with a link to set it up.
