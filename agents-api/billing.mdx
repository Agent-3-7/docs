...
- **\$1 free to start.** Your first visit to the dashboard grants a one-time \$1 credit, enough to run your first instance for days, or a mostly-idle [auto-sleep](/agents-api/instances#auto-sleep) instance for a month or more. Until your first real top-up, the workspace runs one instance of any template on the 2 vCPU / 4 GB shape; a larger shape returns `403 tier_limit`. Your first top-up unlocks the 4/8 and 8/16 shapes, and up to 10 instances; once your top-ups total \$500 the cap rises to 50. See [Instance limits](#instance-limits).
...
The cap rises on its own: your first top-up lifts the workspace to 10 instances, and once your top-ups total \$500 it becomes 50, with nothing to request or configure. Need more than 50? Email [vishnu@agent37.com](mailto:vishnu@agent37.com) or use the chat bubble in the dashboard.
...
Every template offers the same three shapes, and 2 vCPU / 4 GB is the smallest. Disk is any whole number of GB within the shape's range and defaults to the range minimum; disk above the minimum adds \$0.09 per GB per month. See [Instances](/agents-api/instances) for how to pick a shape with `resources` on create.

### Dedicated and shared instances

Every instance you create is **dedicated**: the full shape is reserved for it around the clock, and the prices above are what it bills, flat, every month. There is nothing to set and nothing to pass.

Most agents are idle most of the day. A **shared** instance still bursts to the whole shape whenever it works, but bills that shape's monthly average instead of the reservation, from \$1.99 per month on 2 vCPU / 4 GB and never more than the dedicated price. Isolation, resources, and every endpoint on this reference behave identically; only the capacity reservation changes.

Shared is in beta and is turned on per workspace. Reach for it when cost is the thing you are optimizing: pass `"type": "shared"` on [create](/agents-api/instances#create-an-instance), and before your workspace is enabled the call returns `403 shared_unavailable` with a link to book the onboarding call that turns it on. Leaving `type` out gives you a dedicated instance, which is the right default for anything latency-sensitive or production-facing.
