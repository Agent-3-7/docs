...
Start dedicated. Reach for **shared** when cost is what you are optimizing: pass `"type": "shared"` on [create](/agents-api/instances#create-an-instance). Shared is enabled per workspace; until yours is, create returns `403 shared_unavailable` with a link to set it up.
