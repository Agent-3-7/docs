...
Every row is the same shape regardless of harness: `{ id, title, last_active, message_count, preview }`. `title` is the harness's own editable title (what [rename](#rename-a-session) writes), `last_active` is epoch milliseconds, and fields a harness doesn't track are `null` (OpenClaw tracks no message count or preview).
...
<ResponseField name="context" type="object | null">
  The session's last reported context window: `{ used_tokens, window_tokens }`, the tokens occupying the model's window against the window's size. `null` until a turn reports one.
</ResponseField>
