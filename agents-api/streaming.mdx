...
| `response.completed` | `{ output_text, usage, context }`, terminal; `usage` and `context` can be `null`, and `cost_usd` inside `usage` is `null` when the provider reports no cost |
