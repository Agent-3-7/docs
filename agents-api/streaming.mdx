...
<Info>
  These event names and payloads are the gateway's streaming contract, not a per-agent detail. Hermes, OpenClaw, Claude Code, and Codex emit them today, and the agents that follow will emit the same eight, so your client code does not change when you switch templates.
</Info>
