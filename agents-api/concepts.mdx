...
Two practical consequences. Raw sockets are not available inside an instance, so `ping`, `traceroute` and `tcpdump` do not work — use a TCP or HTTP check instead if your image runs a connectivity self-test at boot. And if you need one instance to talk to another, go through its public URL, not a private address.
