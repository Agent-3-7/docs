...
Nothing is installed in your image. The platform injects the SSH server into the sandbox at runtime, so SSH works on `agent37-hermes`, `agent37-openclaw`, `agent37-claude-code`, `agent37-codex`, and every [custom image](/agents-api/custom-image) you build, including images that ship no `sshd` and no shell tooling of their own.
