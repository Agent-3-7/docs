...
Grok lists the models your xAI API key can run, the `grok-4.x` family; with no key set the list is empty. `reasoning_effort` maps onto Grok's effort levels by name (`ultra` maps to `max`), and non-reasoning models ignore the knob. See [Host Grok](/agents-api/grok).
