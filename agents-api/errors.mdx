...
| `tier_limit` | 403 | Create or resize asks for a shape larger than your plan includes, or a [custom domain](/agents-api/domains) is registered before your top-ups total \$500. Free workspaces (before your first top-up) run any template on the 2 vCPU / 4 GB shape; a top-up unlocks the 4/8 and 8/16 shapes. |
| `shared_unavailable` | 403 | [Create](/agents-api/instances#create-an-instance) asks for `"type": "shared"` on a workspace that isn't enabled for the beta. Nothing is provisioned and nothing is charged. Book the onboarding call named in the message, or omit `type` for a dedicated instance. |
