...
<ResponseField name="error.detail" type="string">
  Optional. Raw failure output, present on a failed create's `provisioning_failed`: the last lines of your container's console log (host paths and IPs redacted, truncated to the final 2,000 characters). The same text persists on the instance as [`status_reason.detail`](/agents-api/instances#the-instance-object).
</ResponseField>
...
| `shared_unavailable` | 403 | [Create](/agents-api/instances#create-an-instance) asks for `"type": "shared"` on a workspace it isn't enabled for. Nothing is provisioned and nothing is charged; book the call in the message. |
...
| `provisioning_failed` | 502/500 | A container or host operation failed (image pull, container start, lifecycle action). A failed create carries `error.detail` with your container's console tail. Creates debit nothing, so a failed create costs nothing. |
...
| `agent_unavailable` | 503 | The targeted harness is not available on this instance: not provisioned here, or down. Hermes missing from an OpenClaw instance reports `hermes_not_found` at 503 instead. |
