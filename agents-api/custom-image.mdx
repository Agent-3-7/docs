...
A complete example — a `Dockerfile`, an example skill, a registration script, and a tiny LLM proxy — lives in [agent37-platform/custom-agent-image](https://github.com/agent37-platform/custom-agent-image). It is a GitHub template repo: click **Use this template**, edit the `Dockerfile`, run `./register.sh`.
...
On GHCR, make the package public after the first push (**Packages** → **Package settings** → **Change visibility** → **Public**). The [example repo](https://github.com/agent37-platform/custom-agent-image) includes a GitHub Actions workflow that builds `linux/amd64` and publishes immutable commit tags.
...
Your endpoint must serve the two OpenAI-compatible routes Hermes uses: `GET /v1/models` (to resolve the model id) and `POST /v1/chat/completions` (the turn). The [example repo](https://github.com/agent37-platform/custom-agent-image) includes a ~40-line `llm-proxy/` that implements exactly these and forwards to OpenRouter with your key — a minimal pattern to deploy and adapt.
